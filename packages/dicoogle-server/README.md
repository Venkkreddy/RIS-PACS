...
## TDAI Offsite Push Integration (Custom)

This monorepo includes additional config/scripts for teleradiology routing:

- `config/central-cstore.xml` - central C-STORE listener config (`TDAI-CENTRAL`, port `104`)
- `src/offsite-route.lua` - local ingest hook that forwards event payloads to central
- `local-config/preconfig.xml` - peer/default forwarding template for local radiographer installs

### Router setup options

1. **Dicoogle peer mode**
   - Configure local node peer to `central.tdairad.com:104`
   - Keep `auto-forward-on-store=true` in local config

2. **External DICOM router (Orthanc or gateway)**
   - Route local C-STORE to the central AET (`TDAI-CENTRAL`)
   - Keep webhook callback to reporting `/webhook/study` for status synchronization

### Flow

1. Local Dicoogle receives C-STORE.
2. `offsite-route.lua` emits routing metadata webhook.
3. Study is pushed to central Dicoogle listener.
4. OHIF reads central data source and reporting app updates study/report status.
