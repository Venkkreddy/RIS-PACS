...
export default function ModeRoute({
  ...
  useEffect(() => {
    ...
    const mapReasonToRoute = (reason?: string) => {
      if (reason === 'NO_SERIES' || reason === 'NO_INSTANCES') {
        return 'imagesPending';
      }
      return 'studyUnavailable';
    };
    ...
    const validateStudies = async () => {
      for (const studyInstanceUID of studyInstanceUIDs) {
        ...
        for (let attempt = 1; attempt <= retryLimit; attempt++) {
          try {
            const qidoRoot = dataSource.getConfig?.()?.qidoRoot?.replace(/\/+$/, '');
            ...
            if (!qidoRoot) {
              ...
              if (!qidoForStudyUID?.length) {
                validationReason = 'STUDY_NOT_FOUND';
              } else {
                validationReason = undefined;
                break;
              }
            } else {
              const validationUrl = `${qidoRoot}/studies/${encodeURIComponent(studyInstanceUID)}/validate`;
              const response = await fetch(validationUrl, {
                method: 'GET',
                credentials: 'include',
              });

              let payload: { reason?: string; isValid?: boolean } | null = null;
              try {
              ...
              validationReason = payload?.reason || (response.status === 404 ? 'STUDY_NOT_FOUND' : 'VALIDATION_ERROR');
            }
        ...
        if (validationReason) {
          const reason = mapReasonToRoute(validationReason);
          navigate(`/notfoundstudy?reason=${encodeURIComponent(reason)}`);
          return;
        }
