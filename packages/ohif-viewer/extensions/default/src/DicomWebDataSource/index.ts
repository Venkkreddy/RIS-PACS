...
*/
  ...
  /**
   * When true, all QIDO/WADO XHRs set withCredentials so the browser sends
   * session cookies to a cross-origin reporting API (e.g. OHIF on :3000 → API on :8081).
   */
...
function createDicomWebApi(dicomWebConfig: DicomWebConfig, servicesManager) {
  ...
  const implementation = {
    initialize: ({ params, query }) => {
      ...
      const crossOriginCredentialHooks =
        dicomWebConfig.crossOriginCookies === true
          ? [
              (request: XMLHttpRequest, _metadata: unknown) => {
      ...
      qidoConfig = {
        ...
        ...(crossOriginCredentialHooks ? { requestHooks: crossOriginCredentialHooks } : {}),
      };
